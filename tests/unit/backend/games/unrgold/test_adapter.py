...
def test_class_level_identity():
    assert UnrGoldGameAdapter.game_id == "unrgold"
    assert UnrGoldGameAdapter.display_name == "Unreal Gold"


def test_build_arguments_without_mods_returns_empty(installed_game, game_config):
    exe, _, _ = installed_game

    adapter = UnrGoldGameAdapter(config=game_config)
...
def test_launch_forwards_mod_ini_flag(installed_game, game_config, recording_launcher):
    ...
    adapter = UnrGoldGameAdapter(config=game_config, process_launcher=recording_launcher)
